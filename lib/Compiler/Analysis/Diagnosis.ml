...
let render =
  ...
  | Annotation_type_mismatch { expected; found } ->
    Printf.sprintf
      ...
      (Kind.show painter expected)
      (Kind.show painter found)
  | Argument_type_mismatch { expected; found } ->
    Printf.sprintf
      "I expected an argument of type %s, but found a %s"
      (Kind.show painter expected)
      (Kind.show painter found)
  | Expected_type kind -> Printf.sprintf "I expect a %s here" (Kind.show painter kind)
